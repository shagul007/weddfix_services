...
jQuery(document).ready(function() {
	jq(function() {		
		jq("#grid").jqGrid(
		{
		   	url : 'load_status_master',
		   	editurl:"edit_status_master",
		   	pageable: true,
		   	loadui:"enable",
			datatype: 'json',
			mtype: 'post',
		   	colNames:['Status Id', 'Status Name','Status Description','Is Active'],
		   	colModel:[
		   		     {name:'id',align:'center', index:'id', width:100,editable:true,hidden:true,editoptions: {readonly:false,hidden:true,size:10}},
                    {name:'statusName',index:'statusName', width:100,editable:true,editrules:{required:true},editoptions:{size:10,maxlength: 30},searchoptions:{sopt: ['cn']}},
                    {name:'statusDesc',index:'statusDesc', width:100,editable:true,editrules:{required:true},editoptions:{size:10,maxlength: 250},searchoptions:{sopt: ['cn']}},
...
<div class="main-container" style="min-height: 430px;">
  <div class="container">
  <div class="row">
	<s:url var="remoteurl" action="load_status_master" id="actionName" />
		<div id="jqgrid">
			<table id="grid"></table>
			<div id="pager"></div>
		</div>
