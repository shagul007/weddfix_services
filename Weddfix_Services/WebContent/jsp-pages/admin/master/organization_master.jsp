...
jQuery(document).ready(function() {
	jq(function() {		
		jq("#grid").jqGrid(
		{
		   	url : 'load_organization_master',
		   	editurl:"edit_organization_master",
		   	pageable: true,
		   	loadui:"enable",
			datatype: 'json',
			mtype: 'post',
		   	colNames:['Organization Id', 'Organization Name','Organization Description','Is Active'],
		   	colModel:[
		   		     {name:'id',align:'center', index:'id', width:100,editable:true,hidden:true,editoptions: {readonly:false,hidden:true,size:10}},
                    {name:'orgName',index:'orgName', width:100,editable:true,editrules:{required:true},editoptions:{size:10,maxlength: 30},searchoptions:{sopt: ['cn']}},
                    {name:'orgDesc',index:'orgDesc', width:100,editable:true,editrules:{required:true},editoptions:{size:10,maxlength: 250},searchoptions:{sopt: ['cn']}},
...
// 		    					jQuery('#orgName',form).attr('readonly','readonly');
		    				},afterSubmit: function (response, postdata) {
		                 		window.location.reload(true);
		                	 } },
		    		        {closeAfterAdd:true,beforeShowForm: function(form) {
// 		    					jQuery('#orgName',form).removeAttr('readonly');		                    
		                  	},
		                  	...
		                  	beforeSubmit:function(postdata,form){	
		        					...
		        					if(re.test(name) == false){
		        		            		  return [false,'Org Name should not have numbers or special characters'];
		        					}}
		        					return [true,','];
...
<div class="main-container" style="min-height: 430px;">
  <div class="container">
  <div class="row">
	<s:url var="remoteurl" action="load_organization_master" id="actionName" />
		<div id="jqgrid">
			<table id="grid"></table>
			<div id="pager"></div>
		</div>
