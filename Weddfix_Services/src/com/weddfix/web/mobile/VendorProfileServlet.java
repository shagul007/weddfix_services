...
public class VendorProfileServlet extends HttpServlet {
        ...
        try {
			...
			if(userId > 0) {
				MyPersonalDetailsFormBean myPersonalDetails = registerService
						.loadVendorPersonalDetails(userId);
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(update.equals("myProfile")) {
			...
			DirectoryVendorProfileFormBean oldUser = registerService
					.loadVendorProfileDetails(Long.parseLong(request.getParameter("id").toString()));
							
				userProfile.setId(Long.parseLong(request.getParameter("id").toString()));
				userProfile.setFullName(request.getParameter("fullName").toString());
				...
				userProfile.setDescription(request.getParameter("aboutYou"));
				userProfile.setFacebookUrl(request.getParameter("facebookUrl"));
				userProfile.setTwitterUrl(request.getParameter("twitterUrl"));
				userProfile.setLinkedinUrl(request.getParameter("linkedinUrl"));
				userProfile.setInstagramUrl(request.getParameter("instagramUrl"));
				userProfile.setPinterestUrl(request.getParameter("pinterestUrl"));
				...
				if (status != null) {
					...
					if(!userProfile.getMobile().toString().equals(oldUser.getMobile().toString())) {
						code = registerService
								.updateVendorMobileVerificationCodeDetails(userProfile.getId());
