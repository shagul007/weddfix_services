...
public class VendorRegisterAction extends ActionSupport implements
		ModelDriven<DirectoryVendorProfileFormBean>, Preparable, ServletRequestAware,
	...
	private Map<Object, Object> planMap;
	...
	DirectoryVendorProfileFormBean vendorProfileFormBean = new DirectoryVendorProfileFormBean();
	...
	public String saveVendorRegisterDetails() {
		HttpSession session = request.getSession(true);
		if (vendorProfileFormBean.getMobile() != null) {
			String pwdHash = null;
			try {
				if (vendorProfileFormBean.getPassword() != null) {
					pwdHash = CommonConstants
							.generateEncryptedPwd(vendorProfileFormBean
									.getPassword());
				}
				if (pwdHash != null) {
					vendorProfileFormBean.setPassword(pwdHash);
					vendorProfileFormBean.setPasswordHash(pwdHash);
				}
				...
				if (vendorProfileFormBean.getId() != null) {
					...
					vendorProfileFormBean.setUpdatedBy(Long.parseLong(session
				...
				Long userId = registerService
						.saveVendorRegisterDetails(vendorProfileFormBean);
				if (userId != null) {
					if (session.getAttribute("update") != null) {
						session.setAttribute("successMessage",
								"Updated Successfully...");
						session.setAttribute("hrefParamSuccess", "vendor_dashboard");
						if(!vendorProfileFormBean.getMobile().toString().equals(session.getAttribute("mobile").toString())) {
							session.setAttribute("mobile", vendorProfileFormBean.getMobile());
							String code = registerService
									.updateVendorMobileVerificationCodeDetails((Long) session.getAttribute("userId"));
							session.setAttribute("verifyMobileNumber", code);
							session.setAttribute("verifiedMobileNumber", false);
						}
						System.out.println("Updated Successfully...");
					} else {
						vendorProfileFormBean.setPassword(null);
						session.setAttribute("successMessage",
								"Vendor Registered Successfully...");
						session.setAttribute("hrefParamSuccess", "vendor_login");
						System.out.println("Vendor Registered Successfully...");
						
						Properties props = new Properties();
				        props.put("fullName", vendorProfileFormBean.getFullName());
			            
			            MailMessage msg = new MailMessage(props, "welcome.vm", vendorProfileFormBean.getEmail(), 
			            		"Welcome to Weddfix");
	...
	public String saveVendorRegisterJsonDetails() {
		HttpSession session = request.getSession(true);
		if (vendorProfileFormBean.getMobile() != null) {
			String pwdHash = null;
			String pwd = null;
			try {
				pwd = vendorProfileFormBean.getPassword();
				if (vendorProfileFormBean.getPassword() != null) {
					pwdHash = CommonConstants
							.generateEncryptedPwd(vendorProfileFormBean
									.getPassword());
				...
				if (userId != null) {
					if (session.getAttribute("update") != null) {
						...
						loginFormBean = loginService.checkLogin(
								vendorProfileFormBean.getEmail(), pwd);
	...
	public String uploadProfilePicture() {
		...
		try {
			...
			Long status = clientInfoService
					.saveVendorProfilePicture(photoGalleryFormBean, getProfilePic());
			if (status != null) {
				if (session.getAttribute("update") != null) {
					session.setAttribute("successMessage",
							"Updated Successfully...");
					session.setAttribute("hrefParamSuccess", "vendor_profile");
					System.out.println("Updated Successfully...");
				} else {
					session.setAttribute("successMessage",
	...
	public String sessionPasswordHash() {
		...
		try {
			...
			LoginFormBean loginFormBean = loginService
					.sessionVendorPasswordHash((Long) session.getAttribute("userId"));
	...
	public String loadRoleAndStatusMasters() {
		...
		planMap = commonMasterService.loadPlan();
	...
	public String checkVendorAlreadyExist() {
		String userExist = commonMasterService.checkVendorAlreadyExist(emailId.toLowerCase());
		if (userExist != null) {
			return "success";
		} else {
			setEmailId(null);
	...
	public String loadAllVendorRoleDetails() {
		List<DirectoryVendorProfileFormBean> userprofileList = commonMasterService
		...
		while (itr.hasNext()) {
			...
			map.put("planId", obj[8]);
			map.put("planName", obj[9]);
	...
	public String sendForgotPassword() {
		HttpSession session = request.getSession(true);
		try {
			DirectoryVendorProfileFormBean userProfile = registerService
					.loadVendorProfileByEmail(emailId);
			...
			props.put("url", getText("url") + "/v_reset?key=" + key);
	...
	public String startPasswordReset(Long userId) throws Exception {
		...
		try {
			DirectoryVendorPwResetFormBean pwResetFormBean = new DirectoryVendorPwResetFormBean();
	...
	public String resetPassword() {
		...
		try {
			...
			commonMasterService.resetVendorPassword(userId, pwdHash);
			session.setAttribute("successMessage",
	...
	public String reSendCode() {
		...
		try {
			if(!getVerifyMobile().equals(session.getAttribute("mobile").toString())) {
				...
				String code = registerService
						.updateVendorMobileVerificationCode((Long) session.getAttribute("userId"));
				session.setAttribute("verifyMobileNumber", code);
			}
		...
		try {
			if(session.getAttribute("verifyMobileNumber").toString().equals(getVerifyCode())) {
				String status = registerService
						.verifyCodeAndUpdateVendorMobile((Long) session.getAttribute("mobile"), (Long) session.getAttribute("userId"));
				if(status != null) {
	...
	public String updateVendorRoleAndStatus() {
		HttpSession session = request.getSession(true);
		Long updatedBy = (Long) session.getAttribute("userId");
		String status = commonMasterService.updateVendorRoleAndStatus(userId,
	...
	public void prepare() throws Exception {
		vendorProfileFormBean = new DirectoryVendorProfileFormBean();
	}

	public DirectoryVendorProfileFormBean getModel() {
		return vendorProfileFormBean;
	}
	...
	public Map<Object, Object> getPlanMap() {
		return planMap;
	}

	public void setPlanMap(Map<Object, Object> planMap) {
